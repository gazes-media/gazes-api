            ...
            var _ref = _async_to_generator(function(request, reply) {
                ...
                return _ts_generator(this, function(_state) {
                    switch(_state.label){
                        case 0:
                            body = request.body;
                            /* verify that every required fields of the body are defined */ if (!body.id) return [
                                2,
                        ...
                        case 2:
                            anime = _state.sent();
                            /* if the anime exists in the database, just update the time and the date else, create a new anime in the database */ if (anime) ref = [
                                body.time,
                                new Date()
                            ], anime.time = ref[0], anime.date = ref[1], ref;
                            else ref1 = [
                                new _Anime.Anime(),
                                body.id,
                                body.time,
                                ...
                                body.episode,
                                new Date()
                            ], anime = ref1[0], anime.id = ref1[1], anime.time = ref1[2], anime.user = ref1[3], anime.duration = ref1[4], anime.episode = ref1[5], anime.date = ref1[6], ref1;
                            _ = reply.send;
                            _tmp = {
                                success: true
                            };
                            return [
                                4,
                                _datasource.AppDataSource.getRepository(_Anime.Anime).save(anime)
                            ];
                        case 3:
                            return [
                                2,
                                _.apply(reply, [
                                    (_tmp.anime = _state.sent(), _tmp)
                                ])
                            ];
                    }
                });
